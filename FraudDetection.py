...
class FraudDetectionApp:
    ...
    def load_credit_card_model(self):
        if not self.credit_card_model:
            self.credit_card_model = joblib.load("Model/ann_model.pkl")
        return self.credit_card_model

    def load_fraud_model(self):
        if not self.fraud_model:
            with open('Model/fraud_detection_model.pkl', 'rb') as model_file:
    ...
    def run(self):
        # Add a sidebar image
        st.sidebar.image("Images/main.png", use_container_width=True)  # Ensure the image file exists
